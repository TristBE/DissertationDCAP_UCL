...
class dcABPFlatWall:
    """
    Dynamical system for dcABP near a flat wall.

    The wall is located at x = 0, keeping the particle to its right (x > 0).
    The wall exerts a repulsive force F_x(x) for x < 0.
    ...
    Equations of motion:
        dx~/dt~ = cos(theta) + F~(x~)
        dtheta/dt~ = 1 + omega~ + F~(x~) * cos(theta)
    """

    def __init__(self, omega_tilde: float = -0.5, k_tilde: float = 1.0):
        """
        Initialize the dcABP flat wall system.

        Args:
            omega_tilde: Rescaled intrinsic frequency. For fixed points to exist,
                         need -1 < omega~ < 0 (i.e., omega and alpha opposite signs, |alpha|v > |omega|)
    ...
    def wall_force(self, x: np.ndarray) -> np.ndarray:
        """
        Harmonic wall force: F~(x~) = -k~ * x~ for x~ < 0, else 0

        This represents a soft repulsive wall at x = 0.
        """
    ...
    def wall_force_derivative(self, x: np.ndarray) -> np.ndarray:
        """
        Derivative of wall force: dF~/dx~ = -k~ for x~ < 0, else 0
        """
        x = np.atleast_1d(np.asarray(x, dtype=float))
        dF = np.zeros_like(x)
        dF[x < 0] = -self.k_tilde
    ...
    def dx_dt(self, x: np.ndarray, theta: np.ndarray) -> np.ndarray:
        ...
        From dcABP equation (10): particle velocity in x-direction
        """
        x = np.atleast_1d(np.asarray(x, dtype=float))
    ...
    def dtheta_dt(self, x: np.ndarray, theta: np.ndarray) -> np.ndarray:
        ...
        From dcABP equation (11): angular velocity (deterministic limit)
        Note: The '1' comes from alpha*v being our timescale (so alpha*v/(alpha*v) = 1)
        """
        x = np.atleast_1d(np.asarray(x, dtype=float))
        theta = np.atleast_1d(np.asarray(theta, dtype=float))
    ...
    def vector_field(self, x: np.ndarray, theta: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """Return the full vector field (dx~/dt~, dtheta/dt~) at given (x~, theta) points."""
...
def find_fixed_points_detailed(system: dcABPFlatWall,
    ...
    """
    Find fixed points with DETAILED information about which branch they come from.

    For flat wall, the fixed point condition gives:
        sin(theta*) = +/- sqrt(-omega~)

    We call these:
        s_plus  = +sqrt(-omega~)   [the '+' branch, sin > 0]
        s_minus = -sqrt(-omega~)   [the '-' branch, sin < 0]

    Returns:
        List of dictionaries, each containing:
        - 'x_star': x position of fixed point
        - 'theta_star': angular position of fixed point
        - 'sin_theta_star': value of sin(theta*)
        - 'cos_theta_star': value of cos(theta*)
        - 'branch': 'plus' or 'minus' (which solution branch)
        - 'valid': whether this is a physically valid fixed point
    """
    ...
    for branch_name, s_value in [('plus', s_plus), ('minus', s_minus)]:
        ...
        # For flat wall, we need cos(theta*) < 0 (particle points into wall)
        # This means theta* must be in (pi/2, 3*pi/2)

        # For sin(theta*) = s > 0: theta* = pi - arcsin(s) is in (pi/2, pi)
        # For sin(theta*) = s < 0: theta* = pi - arcsin(s) is in (pi, 3*pi/2)
        ...
        # Calculate x* from force balance: F~(x*) = -cos(theta*)
        # For x* < 0: F~(x*) = -k~ * x* = -cos(theta*)
        # So: x* = cos(theta*) / k~
...
def find_fixed_points(system: dcABPFlatWall) -> List[Tuple[float, float]]:
    """
    Find fixed points analytically (simple version returning just coordinates).

    Returns:
        List of (x*, theta*) fixed points
    """
...
def compute_jacobian(system: dcABPFlatWall, x: float, theta: float) -> np.ndarray:
    """
    Compute the Jacobian matrix at a point (x, theta).

    J = | dF~/dx~              -sin(theta)        |
        | (dF~/dx~)*cos(theta)  -F~*sin(theta)    |
    """
    x_arr = np.array([x])
...
def classify_fixed_point(jacobian: np.ndarray) -> Dict:
    """
    Classify a fixed point based on eigenvalues of its Jacobian.

    Classification:
    - Stable/Unstable Node: real eigenvalues, same sign
    - Saddle: real eigenvalues, opposite signs (det < 0)
    - Focus: complex eigenvalues
    - Centre: purely imaginary eigenvalues (det > 0, tr = 0)

    Returns dictionary with eigenvalues, trace, determinant, and classification.
    """
    ...
    # Classification by sign of det and trace
    if det < 0:
...
def stability_analysis(system: dcABPFlatWall, verbose: bool = True) -> List[Dict]:
    """
    Perform full stability analysis on all fixed points.

    Methodology:
    1. Find fixed points
    2. Compute Jacobian at each
...
def compute_x_nullcline(system: dcABPFlatWall, theta_range: np.ndarray) -> np.ndarray:
    """
    Compute x-nullcline: dx~/dt~ = 0 -> cos(theta) + F~(x~) = 0

    For x~ < 0: x~ = -cos(theta)/k~
    """
...
def plot_phase_portrait(system: dcABPFlatWall,
    ...
    """
    Construct phase portrait in (x~, theta) plane.
    """
    # Create meshgrid
...
def parameter_sweep_omega(k_tilde: float = 1.0,
    ...
    """Create panel of phase portraits for varying omega~ values."""
...
def parameter_sweep_k(omega_tilde: float = -0.5,
                      ...
                      figsize: Tuple[int, int] = (15, 10)) -> plt.Figure:
    """Create panel of phase portraits for varying k~ values."""
    if k_values is None:
...
def bifurcation_diagram(k_tilde: float = 1.0,
    ...
    """Create bifurcation diagram showing fixed point locations vs omega~."""
...
def integrate_trajectory(system: dcABPFlatWall,
    ...
    """Integrate a trajectory in (x~, theta) space."""
...
def plot_trajectories_on_phase_portrait(system: dcABPFlatWall,
    ...
    """Plot trajectories overlaid on phase portrait."""
...
def classify_trajectory_fate(system: dcABPFlatWall,
    ...
    """
    Classify the fate of a trajectory starting from given initial condition.

    Returns:
        Dictionary with fate classification and details
    """
...
def compute_basin_of_attraction(system: dcABPFlatWall,
    ...
    """
    Compute basin of attraction by classifying fate of trajectories
...
def analyze_sin_theta_flat_wall(omega_range: Tuple[float, float] = (-0.99, -0.01),
    ...
    """
    Analyze sin(theta*) sign correlation with stability for flat wall.
    """
...
def plot_sin_theta_analysis_flat_wall(analysis_data: Dict,
    ...
    """Plot sin(theta*) analysis for flat wall."""
...
def compute_phase_diagram_flat_wall(omega_range: Tuple[float, float] = (-1.5, 0.5),
                                     ...
                                     n_k: int = 100) -> Dict:
    """
    Compute two-parameter phase diagram in (omega~, k~) space for flat wall.
    """
    omega_vals = np.linspace(omega_range[0], omega_range[1], n_omega)
    k_vals = np.linspace(k_range[0], k_range[1], n_k)
...
def plot_phase_diagram_flat_wall(phase_data: Dict,
                                  figsize: Tuple[int, int] = (14, 10)) -> plt.Figure:
    """Plot two-parameter phase diagram for flat wall."""
